...
type ConfigCallFun func() map[string]interface{} // ConfigFunc 动态加载配置信息
var ConfigCallFuns map[string]ConfigCallFun      // ConfigFuncs 先加载到此数组，loadConfig 在动态生成配置信息
...
func init() {
	...
	// 5. 读取环境变量（支持 flags）
	viper.AutomaticEnv()
	ConfigCallFuns = make(map[string]ConfigCallFun)
}
...
func loadConfig() {
	for name, fn := range ConfigCallFuns {
		viper.Set(name, fn())
	}
}
...
// Add 新增配置项
func Add(name string, configFn ConfigCallFun) {

	ConfigCallFuns[name] = configFn
}
...
func GetInterface(path string, defaultValue ...interface{}) interface{} {
	return internalGet(path, defaultValue)
}
